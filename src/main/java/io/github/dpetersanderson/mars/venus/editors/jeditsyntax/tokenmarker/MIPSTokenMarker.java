...
package io.github.dpetersanderson.mars.venus.editors.jeditsyntax.tokenmarker;

import io.github.dpetersanderson.mars.assembler.*;
import io.github.dpetersanderson.mars.mips.instructions.*;
import io.github.dpetersanderson.mars.venus.editors.jeditsyntax.*;
import java.util.*;
...
public class MIPSTokenMarker extends TokenMarker {
                    ...
                    switch (c) {
                        ...
                        case ':':
                            ...
                            */
                            ...
                            try {
                                validIdentifier = io.github.dpetersanderson.mars.assembler.TokenTypes.isValidIdentifier(
    ...
    public ArrayList getTokenExactMatchHelp(Token token, String tokenText) {
        ...
        if (token != null && token.id == Token.KEYWORD1) {
            ArrayList instrMatches = io.github.dpetersanderson.mars.Globals.instructionSet.matchOperator(tokenText);
            if (instrMatches.size() > 0) {
                int realMatches = 0;
                matches = new ArrayList();
                for (int i = 0; i < instrMatches.size(); i++) {
                    Instruction inst = (Instruction) instrMatches.get(i);
                    if (io.github.dpetersanderson.mars.Globals.getSettings().getExtendedAssemblerEnabled()
            ...
            if (keywordTokenText == null && trimmedTokenText.length() > 0) {
                if (trimmedTokenText.charAt(0) == '.') {
                    return getTextFromDirectiveMatch(trimmedTokenText, false);
                        ...
                        >= io.github.dpetersanderson.mars.Globals.getSettings().getEditorPopupPrefixLength()) {
    ...
    private ArrayList getTextFromInstructionMatch(String tokenText, boolean exact) {
        ...
        if (exact) {
            matches = io.github.dpetersanderson.mars.Globals.instructionSet.matchOperator(tokenText);
        } else {
            matches = io.github.dpetersanderson.mars.Globals.instructionSet.prefixMatchOperator(tokenText);
        }
        if (matches == null) {
            return null;
        }
    ...
    public static KeywordMap getKeywords() {
        if (cKeywords == null) {
            ...
            java.util.ArrayList instructionSet =
                    io.github.dpetersanderson.mars.Globals.instructionSet.getInstructionList();
            for (int i = 0; i < instructionSet.size(); i++) {
                cKeywords.add(
                        ((io.github.dpetersanderson.mars.mips.instructions.Instruction) instructionSet.get(i))
                                .getName(),
                        Token.KEYWORD1);
            }
            // add assembler directives
            java.util.ArrayList directiveSet = io.github.dpetersanderson.mars.assembler.Directives.getDirectiveList();
            for (int i = 0; i < directiveSet.size(); i++) {
                cKeywords.add(
                        ((io.github.dpetersanderson.mars.assembler.Directives) directiveSet.get(i)).getName(),
                        Token.KEYWORD2);
            }
            // add integer register file
            io.github.dpetersanderson.mars.mips.hardware.Register[] registerFile =
                    io.github.dpetersanderson.mars.mips.hardware.RegisterFile.getRegisters();
            ...
            io.github.dpetersanderson.mars.mips.hardware.Register[] coprocessor1RegisterFile =
                    io.github.dpetersanderson.mars.mips.hardware.Coprocessor1.getRegisters();
